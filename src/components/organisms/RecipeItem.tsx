...
interface Recipe {
  ...
  image_url: string;
  title: string;
  ingredients: { quantity: string; unit: string; description: string }[];
  publisher: string;
  servings: number;
  cooking_time?: number;
  source_url: string;
...
interface RecipeItemProps {
  favouriteHandler: (id: string | undefined) => void;
  savedItems: Recipe[];
}

const RecipeItem = ({ favouriteHandler, savedItems }: RecipeItemProps) => {
  const [itemSavedStatus, setItemSavedStatus] = useState<boolean | null>(null);
  const { id } = useParams<{ id?: string }>();

  const { data: recipe, loading, error } = useFetch<Recipe>(id || "");

  const durationCalc = (duration: number): string | undefined => {
